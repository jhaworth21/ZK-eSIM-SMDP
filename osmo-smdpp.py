...
class SmDppHttpServer:
    ...
    def __init__(self, server_hostname: str, ci_certs_path: str, common_cert_path: str,
        ...
        # ZK authorization tokens are one-time use across SM-DP+ sessions.
    ...
    def initiateAutentication(self, request: IRequest, content: dict) -> dict:
        ...
        # Strip optional port suffix (e.g. "host:8443") before comparing — spec uses bare FQDN.
        rcv_host = content['smdpAddress'].split(':')[0]
        if rcv_host.lower() != self.server_hostname.lower():
    ...
    def authenticateClient(self, request: IRequest, content: dict) -> dict:
        ...
        if self.zk_mode:
            ...
            ss.h_pid = elig['hashedPseudonym']
            ss.sig_cred = elig['credentialSignature']
            ss.auth_tok = elig['authorizationToken']
            ss.root_auth = elig['authorizationRoot']
            ss.sig_root = elig['rootSignature']
            ss.pi_inc_bytes = elig['inclusionProof']
        ...
        if self.zk_mode:
            ...
            if not rsp.MerkleAccumulator.verifyProof(ss.h_pid.hex(), ss.pi_inc, ss.root_auth):
                raise ApiError('0.1', '1.3', 'Failed to Verify Inclusion Proof')

            # Spend token once eligibility is validated.
            spent_key = ss.auth_tok.hex()
            if spent_key in self.zk_spent_tokens:
                raise ApiError('0.1', '1.2', 'Authorization token already spent')
            self.zk_spent_tokens.add(spent_key)
